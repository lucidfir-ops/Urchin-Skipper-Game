...
The minimal repository is verified and published. In the full authoring workspace, synchronize the selected files into `.github-backup/checkout/`, verify there, and commit/push from that repository. Do not push the full workspace's older large import history. Include new required source/assets in the selection as the game evolves; do not rely on a stale initial file list or assume the prepared copy updates itself. Keep the source and copied policy/status documents consistent. In an ordinary GitHub clone, use that clone normally. See [current backup status](PROJECT_STATUS.md#github-backup-and-release-workflow).
