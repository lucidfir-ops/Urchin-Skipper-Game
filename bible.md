...
Taxis travel around 30 knots, avoid boats but ignore underwater bubbles, and can strike a surfaced diver under the ordinary injury rules. September 24: choose a realistic long haul across the map at spawn. A tunable share may cross the divers’ working area, but ordinary routes do not require or seek a fishing patch. Commit each course rather than homing on moving divers. Adjust route-crossing probability or spawn count during playtesting to tune interference; initial working-area opportunity is 30%, keeping the existing taxi cadence. Warn on an actual close fast pass near a surfaced diver. The player can position to influence traffic. Nine-ships tourists move around 5 knots and avoid bubbles. Keep spawns, routes and encounters configurable.
