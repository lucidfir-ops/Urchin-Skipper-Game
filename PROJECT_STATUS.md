...
The [minimal checkout](docs/github-backup-review/README.md) is prepared at `.github-backup/checkout/` with its own Git repository, separate from the full workspace's existing import history. Fresh `npm ci`, lint, formatting, 410 passing tests (one private-save regression skipped) and production build pass there. Exactly 533 selected files are visible to Git; generated/dependency/private files are ignored. Initial local commit: `7faf5af6db726b29af42e2b3c0141b9b7649a321`. Public visibility is confirmed by the designer's screenshot. The first push failed before authentication. GitHub CLI browser sign-in is now complete as `lucidfir-ops`; the repository API confirms public visibility and push permission, and Git uses the installed CLI credential helper. Nothing has been uploaded. Next step: sync the latest setup notes into the minimal checkout, commit them and retry publication, one step at a time. Publish from the prepared repository, not the full workspace's large import history.

GitHub CLI 2.101.0 is installed locally at `.runtime/github-cli/2.101.0/bin/gh`; the official release archive checksum and `gh --version` were verified. Browser authentication and HTTPS Git credential-helper configuration are complete. The CLI reported file-based credential storage; credentials are outside the selected backup. No system installation was needed.
